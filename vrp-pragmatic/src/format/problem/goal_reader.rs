...
fn get_tour_limit_feature(
    ...
    let (distances, durations, activity_durations) = api_problem
        .fleet
        .vehicles
        .iter()
        .filter_map(|vehicle| vehicle.limits.as_ref().map(|limits| (vehicle, limits)))
        .fold((HashMap::new(), HashMap::new(), HashMap::new()), |(mut distances, mut durations, mut activity_durations), (vehicle, limits)| {
            ...
            limits.max_activity_duration.iter().for_each(|max_activity_duration| {
                activity_durations.insert(vehicle.type_id.clone(), *max_activity_duration);
            });

            (distances, durations, activity_durations)
        });
